<template>
  <div id="project">
    <app-navbar :projects="projects" :currentImage="currentImage"></app-navbar>
    <h3 id="project-name">projet : {{ projects[id -1].name.toLowerCase()}}</h3>
    <div class="card">
      <div id="project-image">
        <img
          @click="showModal()"
          :src="'/img/' + projects[id -1].image[currentImage]"
      ...
      <div id="project-image-pagination">
        ...
        <span>{{ currentImage }} / {{ projects[id -1].image.length - 1}}</span>
        <button @click="next()">
      ...
      <div class="card-action">
        <ul>
          <li>
            <a :href="projects[id -1].demo" target="_blank" rel="noopener">demo</a>
          </li>
          <li>
            <a :href="projects[id -1].code" target="_blank" rel="noopener">code</a>
          </li>
        </ul>
      </div>
      <div id="about-project">
        <h4>A propos du projet</h4>
        <div id="description" v-html="projects[id -1].description"></div>
      </div>
      ...
      <div id="technologies">
        ...
        <ul>
          <li
            v-for="(technology,index) of projects[id -1].used_technologies"
            :key="index"
          >{{ technology }}</li>
        </ul>
      </div>
      ...
      <div id="year">
        <h4>Année</h4>
        <p>{{ projects[id -1].year_of_creation }}</p>
      </div>
    </div>
    <div @click="closeModal($event)" class="modal">
      <div class="full-img">
        <img :src="'/img/' + projects[id -1].image[currentImage]" />
...
export default {
    ...
    return {
      ...
      projects: [
        ...
        },
        {
          ...
          image: [
            "test.jpg",
            "les-pays-du-monde.png",
            "les-pays-du-monde-2.png"
          ],
        ...
        },
        {
          ...
          name: "Mon Portfolio",
          description: `Mon portfolio, réalisé afin de centraliser mes travaux et les rendre accessibles publiquement.`,
          overview:
  ...
  methods: {
    prev() {
      if (this.currentImage <= 1) {
        this.currentImage = this.projects[this.id - 1].image.length;
      }
      this.currentImage--;
    },

    next() {
      if (this.currentImage >= this.projects[this.id - 1].image.length - 1) {
        this.currentImage = 0;
      }
      this.currentImage++;
    },
  ...
  created() {
    EventBus.$on("currentProject", data => {
      this.id = data;
    });

    EventBus.$on("currentImage", data => {
